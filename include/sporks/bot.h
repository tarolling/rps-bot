 ...
 *
 * There are two caching mechanisms for discord data, first is aegis itself, and
 *the second level is a mysql databse, which exists to feed information to the
 *dashboard website and give some persistence to the data. Note that from the
 *bot's perspective this data is write-only and the bot will only ever look at
 *the data in the bot, and from the website's perspective this data is read-only
 *and it will look to the discord API for authentication and current server list
 *of a user.
 *
 * All major parts of this bot are modular, and can be hot-reloaded on the fly
 *to prevent having to restart the shards. Please see the modules directory for
 *source code.
 *
 ************************************************************************************/
...
public:
  ...
  Bot(Bot &&other) noexcept;       // move constructor
  Bot &operator=(const Bot &other) // copy assignment
  {
    return *this = Bot(other);
  }
  Bot &operator=(Bot &&other) noexcept; // move assignment
  ...
  void
  onMessageReactionRemoveAll(const dpp::message_reaction_remove_all_t &event);
