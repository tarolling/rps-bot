...
/** Implementation-specific flags which may be set in Module constructor by
 * calling Attach()
 */
enum Implementation {
  I_BEGIN,
...
/**
 * This #define allows us to call a method in all loaded modules in a readable
 * simple way, e.g.: 'FOREACH_MOD(I_OnGuildAdd,OnGuildAdd(guildinfo));' NOTE:
 * Locks mutex - two FOREACH_MOD() can't run asyncronously in two different
 * threads. This is to prevent one thread loading/unloading a module, changing
 * the arrays/vectors while another thread is running an event.
 */
  ...
  {                                                                            \
    ...
    for (auto _i = list_to_call.begin(); _i != list_to_call.end(); ++_i) {     \
      try {                                                                    \
        ...
        core->log(dpp::ll_error, fmt::format("Exception caught in module: {}", \
                                             modexcept.what()));               \
      }                                                                        \
    }                                                                          \
  };
...
using initfunctype = Module *(Bot *, ModuleLoader *);

/** A map representing the list of modules as external systems see it */
using ModMap = std::map<std::string, Module *>;

/**
 * ModuleNative contains the OS level details of the module, e.g. the handle
 * returned by dlopen() and the last error message string, also a pointer to the
 * init_module() function within the module.
 */
struct ModuleNative {
  initfunctype *init;
  void *dlopen_handle;
...
/**
 * ModuleLoader handles loading and unloading of modules at runtime, and
 * maintains a list of loaded modules. It can be queried for this list.
 */
class ModuleLoader {
  Bot *bot;
...
public:
  ...
  ModuleLoader(const ModuleLoader &other);           // copy constructor
  ModuleLoader(ModuleLoader &&other) noexcept;       // move constructor
  ModuleLoader &operator=(const ModuleLoader &other) // copy assignment
  {
    return *this = ModuleLoader(other);
  }
  ModuleLoader &operator=(ModuleLoader &&other) noexcept; // move assignment

  /* Attach a module to an event. Only events a module explicitly attaches to
   * will be called for that module, this allows a module to turn events on and
   * off as it needs on the fly.
   */
  ...
  /* Load a module from a shared object file. The path is relative to the bot's
   * executable.
   */
  bool Load(const std::string &filename);

  /* Unload a module from memory. Calls the Module class's destructor and then
   * dlclose().
   */
  ...
  /* Enumerate modules within modules.json and load them all. Any modules that
   * are already loaded will be ignored, so we can use this to load new modules
   * on rehash
   */
  void LoadAll();
...
/**
 * All modules that can be loaded at runtime derive from the Module class. This
 * class contains virtual methods for each event triggered by the library that
 * the bot handles internally, plus some helper methods to identify and report
...
public:
  /* Instantiate a module, the ModuleLoader is passed in so that we can attach
   * events */
  Module(Bot *instigator, ModuleLoader *ml);
  virtual ~Module();
  Module(const Module &other);           // copy constructor
  Module(Module &&other) noexcept;       // move constructor
  Module &operator=(const Module &other) // copy assignment
  {
    return *this = Module(other);
  }
  Module &operator=(Module &&other) noexcept; // move assignment
  ...
  virtual bool
  OnMessageReactionRemove(const dpp::message_reaction_remove_t &obj);
  virtual bool
  OnMessageReactionRemoveAll(const dpp::message_reaction_remove_all_t &obj);
  ...
  virtual bool
  OnGuildIntegrationsUpdate(const dpp::guild_integrations_update_t &obj);
  ...
  /* Emit a simple text only embed to a channel, many modules use this for error
   * reporting */
...
#define ENTRYPOINT(mod_class_name)                                             \
  extern "C" Module *init_module(Bot *instigator, ModuleLoader *ml) {          \
    return new mod_class_name(instigator, ml);                                 \
  }
