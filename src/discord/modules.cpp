 ...
 */
const char *StringNames[I_END + 1] = {"I_BEGIN",
                                      "I_OnMessage",
                                      "I_OnReady",
                                      "I_OnChannelCreate",
                                      "I_OnChannelDelete",
...
ModuleLoader::ModuleLoader(Bot *creator) : bot(creator) {
  bot->core->log(dpp::ll_info, "Module loader initializing...");
}

ModuleLoader::~ModuleLoader() {}

/**
 * Attach an event to a module. Rather than just calling all events at all
 * times, an event can be enabled or disabled with Attach() and Detach(), this
 * allows a module to programatically turn events on and off for itself.
 */
void ModuleLoader::Attach(const std::vector<Implementation> &i, Module *mod) {
  for (auto n = i.begin(); n != i.end(); ++n) {
    if (std::find(EventHandlers[*n].begin(), EventHandlers[*n].end(), mod) ==
        EventHandlers[*n].end()) {
      ...
      bot->core->log(
          dpp::ll_warning,
          fmt::format("Module \"{}\" is already attached to event \"{}\"",
                      mod->GetDescription(), StringNames[*n]));
    }
  }
}
...
void ModuleLoader::Detach(const std::vector<Implementation> &i, Module *mod) {
  for (auto n = i.begin(); n != i.end(); ++n) {
    auto it =
        std::find(EventHandlers[*n].begin(), EventHandlers[*n].end(), mod);
    if (it != EventHandlers[*n].end()) {
      EventHandlers[*n].erase(it);
      bot->core->log(dpp::ll_debug,
                     fmt::format("Module \"{}\" detached event \"{}\"",
                                 mod->GetDescription(), StringNames[*n]));
    }
  }
}
 ...
 */
const ModMap &ModuleLoader::GetModuleList() const { return ModuleList; }
...
/**
 * Returns the last error caused by Load() or Unload(), or an empty string for
 * no error.
 */
const std::string &ModuleLoader::GetLastError() { return lasterror; }
...
bool ModuleLoader::Unload(const std::string &filename) {
  ...
  bot->core->log(dpp::ll_info,
                 fmt::format("Unloading module {} ({}/{})", filename,
  ...
  for (int j = I_BEGIN; j != I_END; ++j) {
    auto p = std::find(EventHandlers[j].begin(), EventHandlers[j].end(),
                       mod.module_object);
    if (p != EventHandlers[j].end()) {
      EventHandlers[j].erase(p);
      bot->core->log(dpp::ll_debug, fmt::format("Removed event {} from {}",
                                                StringNames[j], filename));
    }
  }
  /* Remove module entry */
  Modules.erase(m);
  ...
  bot->core->log(dpp::ll_debug, fmt::format("New module counts: {}/{}",
                                            Modules.size(), ModuleList.size()));
...
/**
 * Unload, then reload a loaded module. Returns true on success or false on
 * failure. Failure to unload causes load to be skipped, so you can't use this
 * function to load a module that isnt loaded.
 */
...
/**
 * Return a given symbol name from a shared object represented by the
 * ModuleNative value.
 */
bool ModuleLoader::GetSymbol(ModuleNative &native, const char *sym_name) {
  ...
  if (native.dlopen_handle != nullptr) {
    ...
    native.err = dlerror();
    // printf("dlopen_handle=0x%016x, native.init=0x%016x native.err=\"%s\"
...
std::string Module::GetVersion() { return ""; }

std::string Module::GetDescription() { return ""; }

bool Module::OnChannelCreate(const dpp::channel_create_t &channel) {
  return true;
}

bool Module::OnReady(const dpp::ready_t &ready) { return true; }

bool Module::OnChannelDelete(const dpp::channel_delete_t &channel) {
  return true;
}

bool Module::OnGuildCreate(const dpp::guild_create_t &guild) { return true; }

bool Module::OnGuildDelete(const dpp::guild_delete_t &guild) { return true; }

bool Module::OnGuildMemberAdd(const dpp::guild_member_add_t &gma) {
  return true;
}

bool Module::OnMessage(const dpp::message_create_t &message,
                       const std::string &clean_message, bool mentioned,
                       const std::vector<std::string> &stringmentions) {
  return true;
}

bool Module::OnPresenceUpdate() { return true; }

bool Module::OnTypingStart(const dpp::typing_start_t &obj) { return true; }

bool Module::OnMessageUpdate(const dpp::message_update_t &obj) { return true; }

bool Module::OnMessageDelete(const dpp::message_delete_t &obj) { return true; }

bool Module::OnMessageDeleteBulk(const dpp::message_delete_bulk_t &obj) {
  return true;
}

bool Module::OnGuildUpdate(const dpp::guild_update_t &obj) { return true; }

bool Module::OnMessageReactionAdd(const dpp::message_reaction_add_t &obj) {
  return true;
}

bool Module::OnMessageReactionRemove(
    const dpp::message_reaction_remove_t &obj) {
  return true;
}

bool Module::OnMessageReactionRemoveAll(
    const dpp::message_reaction_remove_all_t &obj) {
  return true;
}

bool Module::OnUserUpdate(const dpp::user_update_t &obj) { return true; }

bool Module::OnResumed(const dpp::resumed_t &obj) { return true; }

bool Module::OnChannelUpdate(const dpp::channel_update_t &obj) { return true; }

bool Module::OnChannelPinsUpdate(const dpp::channel_pins_update_t &obj) {
  return true;
}

bool Module::OnGuildBanAdd(const dpp::guild_ban_add_t &obj) { return true; }
...
bool Module::OnGuildIntegrationsUpdate(
    const dpp::guild_integrations_update_t &obj) {
  return true;
}
